...
from ._rpc import rpc_pb2, rpc_pb2_grpc
...
EtcdResponseCallback = typing.Callable[
    [KeepAliveResponse], typing.Coroutine[typing.Any, typing.Any, None]
]
...
class KeepAliveLease:
    ...
    def __init__(
        ...
        ttl: int,
        *,
        etcd_response_callback: typing.Optional[EtcdResponseCallback] = None,
    ):
        self.client = client
        self.ttl = ttl
    ...
    async def _run_keep_alive_task(self) -> None:
        ...
        async for response in self.client.keep_alive(
            ...
            if self._response_callback is not None:
                asyncio.get_event_loop().create_task(self._response_callback(response))
...
async def _keep_alive_request_generator(
    lease_id: int, period: int, finisher: typing.Optional[asyncio.Event] = None
...
class LeaseApi:
    ...
    async def revoke(self, id: int) -> LeaseRevokeResponse:
        ...
        return LeaseRevokeResponse(header=ResponseHeader.from_protobuf(r.header))

    async def time_to_live(self, id: int, keys: bool = False) -> TimeToLiveResponse:
    ...
    def _keep_alive(
        self,
        request_iterator: typing.AsyncGenerator[rpc_pb2.LeaseKeepAliveRequest, None],
    ...
    async def keep_alive(
        ...
        async for result in self._keep_alive(
            _keep_alive_request_generator(lease_id, period, finisher)
        ):
    ...
    def keep_alive_context(
        ...
        ttl: int,
        *,
        response_callback: typing.Optional[EtcdResponseCallback] = None,
    ) -> KeepAliveLease:
        """Keeps the lease alive by streaming keep alive requests from the client
        to the server and streaming keep alive responses from the server to
        the client.
        """
        return KeepAliveLease(self, ttl, etcd_response_callback=response_callback)
